...
def capture_image_from_camera():
    """Capture an image from the camera with resizable window."""
    ...
    cv2.namedWindow("Camera (Press 'c' to capture, 'q' to quit)", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("Camera (Press 'c' to capture, 'q' to quit)", window_width, window_height)
    ...
    while True:
        ...
        cv2.imshow("Camera (Press 'c' to capture, 'q' to quit)", resized_frame)
        ...
        window_width, window_height = cv2.getWindowImageRect("Camera (Press 'c' to capture, 'q' to quit)")[2:]
...
def main():
    ...
    print("2. EasyOCR (for English, coding, history, other languages/frq's that are non-math)")
